...
public class SemanticAnalyzer {
     ...
     * 一个项目 一次构造
     */
    public SemanticAnalyzer(ProblemCollector collector) {
...
//        ast.accept(visitor);
     ...
     * 建议优先使用
     */
    private void newScope(Runnable cons) {
        enterScope();
        cons.run();
        exitScope();
    }
    ...
    private Problem.SemanticProblem warning(String name) {
        Problem.SemanticProblem w = new Problem.SemanticProblem(sourceMap, name, Problem.ProblemLevel.WARNING);
    ...
    /*public class AnalysisVisitor implements SemanticVisitor {
        ...
        public void visit(IfStmt node) {
            if(node.condition.accept(this) != Bool) {
                throw error("while语句的条件类型不为Bool")
                        .point(node.condition, "");
            }
            newScope(() -> {
                ...
                if(node.expr instanceof Literal literal) {
                    if(literal.token.literal instanceof Double repeatNum) {
                        if(node.varDecl != null)
                            error("for语句中参数为重复次数时不能声明变量")
                                    .info(node.expr, "参数: 重复次数")
                                    .point(node.varDecl, "变量声明");
                        if(repeatNum <= 0)
                            error()
                    }
                }
